  ...
  <div>
    <div class="lr df">
      <div style="background: #7ccabf" class="gou">
        <div style="width: 90%">今日发布<br />{{ getcount }}</div>
        <i class="el-icon-check"></i>
      </div>
      <div style="background: #e78a86" class="gou">
        <div style="width: 90%">原创<br />{{ getoriginal }}</div>
...
export default {
  ...
  mounted() {
    this.getData();
    this.arr.map(item=>{
    this.chartData.columns=Object.keys(item.category)
    this.chartData.rows=item
    })
  },
  computed: {
    getcount() {
